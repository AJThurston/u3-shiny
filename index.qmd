...
pdf_post <- pnorm(z_post)
  ...
  data_ann <- data.frame(
    group = factor(c("H0", "Ha")),
    labels = c(
      paste0(input$before_label, nround(input$base_rate*100, digits = 0), "%"),
      paste0(input$after_label, nround(pdf_post*100, digits = 0), "%")
    )
  )
...
ggplot(data, aes(x = z_values, y = density, fill = group, color = group)) +
  ...
  scale_fill_manual(values = c(input$after_color,input$before_color)) +
  scale_color_manual(values = c(input$after_color,input$before_color)) +
